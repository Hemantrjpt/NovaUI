...
-- Lays out same-height elements side by side (e.g. "Save" / "Reset" as a pair)
-- instead of the section's default full-width vertical stacking. Works best
-- with Button/Toggle/Dropdown/Keybind/Textbox/ColorPicker (all 34px tall);
-- mixing in a Slider or Paragraph will look uneven since they're taller.
function NovaUI:CreateRow(columns)
	local theme = self._theme
	columns = columns or 2
	local cellScale = (1 / columns) - (0.02 * (columns - 1))
	...
	local row = Create("Frame", {
		...
		Create("UIGridLayout", {
			CellSize = UDim2.new(cellScale, 0, 0, 34),
	...
	return setmetatable({_theme = theme, _container = row, _screenGui = self._screenGui}, NovaUI)
end
...
function NovaUI:CreateSlider(opts)
	...
	local trackInsetL, trackInsetR = 0, 0
	if opts.LeftIcon then
		local leftIconInst = CreateIcon(opts.LeftIcon, 16, theme.SubTextColor)
		if leftIconInst then
			leftIconInst.Position = UDim2.new(0, 0, 0, trackY - 5)
			leftIconInst.Parent = holder
			trackInsetL = 22
		end
	end
	if opts.RightIcon then
		local rightIconInst = CreateIcon(opts.RightIcon, 16, theme.SubTextColor)
		if rightIconInst then
			rightIconInst.AnchorPoint = Vector2.new(1, 0)
			rightIconInst.Position = UDim2.new(1, 0, 0, trackY - 5)
			rightIconInst.Parent = holder
			trackInsetR = 22
		end
	end

	local track = Create("Frame", {
		Position = UDim2.new(0, trackInsetL, 0, trackY),
		Size = UDim2.new(1, -14 - trackInsetL - trackInsetR, 0, 6),
		BackgroundColor3 = theme.Sidebar,
		Parent = holder,
	}, {Corner(3)})
